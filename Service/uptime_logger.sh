...
if [ $DEBUG = true ] && [[ "$*" == *"--exit-with-update"* ]]; then
    ...
    trap finishwithUpdate INT TERM
else
    trap "allowShutdown; exit;" INT TERM
fi
...
# activetimeByCount: [0-9]+ (seconds interval)
# activetimeByDiff: [0-9]+ (seconds interval)
...
while true; do
    ...
    echo $activetimecount >"$LOGS_DIR/activetimecount"
    sleep 1
done &
...
while true; do
    ...
    if [ $diff -gt 0 ] && [ $diff -le 5 ]; then
        ...
        lasttimestep=$now
        echo $activetime >"$LOGS_DIR/activetime"
    elif [ $diff -gt 5 ]; then
        lasttimestep=$now
    fi
    sleep 0.2
...
while true; do
    ...
    LOG+="activetimeByCount: $(cat "$LOGS_DIR/activetimecount")"$'\n'
    LOG+="activetimeByDiff: $(($(date +%s) - scriptinit))"$'\n'
